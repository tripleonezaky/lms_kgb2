<?php
// guru/nilai/rekap_uas.php (placeholder awal, struktur mirip UTS)
...
function get_assignments_guru_rekap($guru_id){
    $sql = "SELECT ag.id, ag.kkm, ag.kelas_id, ag.mapel_id, ag.tahun_ajaran_id, k.nama_kelas, mp.nama_mapel, ta.nama_tahun_ajaran
            ...
            WHERE ag.guru_id = ".(int)$guru_id." ORDER BY ta.id DESC, k.nama_kelas ASC, mp.nama_mapel ASC";
...
function fetch_kontext($assignment_id){
    $res = query("SELECT ag.*, k.nama_kelas, mp.nama_mapel, ta.nama_tahun_ajaran FROM assignment_guru ag JOIN kelas k ON k.id=ag.kelas_id JOIN mata_pelajaran mp ON mp.id=ag.mapel_id JOIN tahun_ajaran ta ON ta.id=ag.tahun_ajaran_id WHERE ag.id=".(int)$assignment_id." LIMIT 1");
...
function nilai_uas($siswa_id,$mapel_id,$kelas_id,$tahun_ajaran_id,$semester){
    $sql = "SELECT ROUND(AVG(n.nilai),2) AS avg_val FROM nilai n WHERE n.siswa_id={$siswa_id} AND n.mapel_id={$mapel_id} AND n.kelas_id={$kelas_id} AND n.tahun_ajaran_id={$tahun_ajaran_id} AND n.semester='".escape_string($semester)."' AND n.jenis_penilaian='UAS'";
    $res=query($sql); $row=$res?fetch_assoc($res):null; return $row&&$row['avg_val']!==null? (float)$row['avg_val']: null;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_kkm'])) {
    $aid = (int)($_POST['assignment_id'] ?? 0); $kkm=(int)($_POST['kkm'] ?? 0);
    if ($aid>0 && $kkm>0) {
        $cek = query("SELECT id FROM assignment_guru WHERE id={$aid} AND guru_id={$guru_id} LIMIT 1");
        if ($cek && fetch_assoc($cek)) { $ok = query("UPDATE assignment_guru SET kkm={$kkm} WHERE id={$aid} LIMIT 1"); set_flash($ok? 'success':'error', $ok? 'KKM disimpan.':'Gagal menyimpan KKM.'); }
        else set_flash('error','Assignment tidak valid.');
    } else set_flash('error','KKM tidak valid.');
    $qs = http_build_query(['assignment_id'=>$aid,'semester'=>$semester]); redirect('rekap_uas.php?'.$qs);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_bulk'])) {
    ...
    $uasArr = isset($_POST['uas']) && is_array($_POST['uas']) ? $_POST['uas'] : [];
    $updated = 0;
    ...
    foreach ($uasArr as $sid => $v) {
        $sid = (int)$sid; $val = trim((string)$v); if ($val==='') continue;
        $num = (float)str_replace(',', '.', $val);
        $cek = query("SELECT id FROM nilai WHERE siswa_id={$sid} AND mapel_id={$mapel_id} AND kelas_id={$kelas_id} AND tahun_ajaran_id={$tahun_ajaran_id} AND semester='".escape_string($semester)."' AND jenis_penilaian='UAS' LIMIT 1");
        $rowC = $cek? fetch_assoc($cek): null;
        if ($rowC) query("UPDATE nilai SET nilai={$num}, updated_at='".date('Y-m-d H:i:s')."' WHERE id=".(int)$rowC['id']);
        else query("INSERT INTO nilai (siswa_id,mapel_id,kelas_id,tahun_ajaran_id,semester,jenis_penilaian,nilai,created_at,updated_at) VALUES ({$sid},{$mapel_id},{$kelas_id},{$tahun_ajaran_id},'".escape_string($semester)."','UAS',{$num},'".date('Y-m-d H:i:s')."','".date('Y-m-d H:i:s')."')");
        $updated++;
    }
    set_flash('success','Tersimpan. Baris diproses: '.$updated);
    $qs = http_build_query(['assignment_id'=>$assignment_id,'semester'=>$semester]); redirect('rekap_uas.php?'.$qs);
}
...
if ($ctx && isset($_GET['export_csv']) && (int)$_GET['export_csv']===1) {
    ...
    header('Content-Disposition: attachment; filename=rekap_uas_'.preg_replace('/\s+/','_', $ctx['nama_mapel']).'_'.preg_replace('/\s+/','_', $ctx['nama_kelas']).'_'.strtolower($semester).'.csv');
...
if ($ctx && isset($_GET['export']) && (int)$_GET['export']===1) {
    ...
    header('Content-Disposition: attachment; filename=rekap_uas_'.preg_replace('/\s+/','_', $ctx['nama_mapel']).'_'.preg_replace('/\s+/','_', $ctx['nama_kelas']).'_'.strtolower($semester).'.xls');
    ...
    $no=1; foreach ($rows as $r){
        ...
        echo '<tr>'
            ...
            .'<td>'.($uas!==null? number_format($uas,2) : '').'</td>'
            .'<td>'.htmlspecialchars((to_predikat($uas) ?? ''), ENT_QUOTES, 'UTF-8').'</td>'
...
<head>
  ...
  <title>Rekap Nilai UAS - Guru</title>
...
<body>
  <div class="container">
    <div class="card">
      <div class="card-header">Rekap Nilai UAS</div>
      <div class="card-body">
        ...
        <?php $assignments=get_assignments_guru_rekap($guru_id); if(!$assignments): ?>
            <div class="muted">Anda belum memiliki assignment. Minta admin untuk menambahkan assignment_guru.</div>
        <?php else: ?>
        <form method="get" action="rekap_uas.php" class="row" style="align-items:flex-end;margin-bottom:12px">
          <div class="col">
        ...
        <?php if ($ctx): ?>
          ...
          <div class="scroll-x">
            <table class="table">
              <thead>
                <tr>
                  <th>No.</th><th>NISN</th><th>Nama Siswa</th><th>KKM</th><th>Formatif</th><th>Predikat</th><th>UAS</th><th>Predikat</th><th>Keterangan</th>
                </tr>
              </thead>
              <tbody>
                <?php if (!$siswa_list): ?>
                ...
                ?>
                  <tr>
                    ...
                    <td style="text-align:center"><input type="number" step="0.01" min="0" max="100" name="uas[<?php echo (int)$s['id']; ?>]" value="<?php echo $uas!==null? htmlspecialchars(number_format($uas,2,'.','')) : ''; ?>" style="width:100px;padding:6px;border:1px solid #cbd5e1;border-radius:6px;text-align:right"></td>
                    <td style="text-align:center"><?php echo htmlspecialchars((to_predikat($uas) ?? '')); ?></td>
