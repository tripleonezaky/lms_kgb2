<?php
/**
 * File: guru/nilai/tugas.php
 * Fitur: Penilaian Tugas oleh Guru
 * - Menampilkan daftar pengumpulan tugas (submitted) untuk assignment milik guru
 * - Memungkinkan guru memberi nilai dan feedback sederhana
 */
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nilai_id'])) {
    ...
    // Validasi: pastikan pengumpulan berasal dari tugas milik guru
    $sql_check = "
        SELECT pt.id
        FROM pengumpulan_tugas pt
        JOIN tugas t ON pt.tugas_id = t.id
        JOIN assignment_guru ag ON t.assignment_id = ag.id
        WHERE pt.id = {$pengumpulan_id} AND ag.guru_id = {$guru_id}
        LIMIT 1
    ";
    $res_check = query($sql_check);
    if ($res_check && fetch_assoc($res_check)) {
    $nilai_val = $nilai !== null ? number_format($nilai, 2, '.', '') : 'NULL';
    $sql_upd = "UPDATE pengumpulan_tugas SET nilai = {$nilai_val}, status = 'graded' WHERE id = {$pengumpulan_id}";
    if (query($sql_upd)) {
    // Upsert ke tabel nilai sebagai Nilai Harian
    ...
    set_flash('success', 'Penilaian berhasil disimpan.');
    } else {
...
// Handle minta revisi (needs_revision)
...
// Ambil daftar pengumpulan tugas yang submitted untuk guru ini
$sql_list = "
    SELECT pt.id, pt.tanggal_submit, pt.status, pt.file_path, pt.isi_jawaban,
    ...
    WHERE ag.guru_id = {$guru_id} AND pt.status IN ('submitted','needs_revision','graded','late')
...
<head>
    ...
    <style>
        ...
        table { width:100%; border-collapse: collapse; table-layout:auto; min-width: 1280px; }
        tbody td { word-wrap:break-word; }
        th, td { border-bottom:1px solid #eee; padding:10px; text-align:left; vertical-align:top; }
        th { background:#f8fafc; }
        /* Responsif: scroll mendatar jika kolom banyak */
        .table-responsive{ overflow-x:auto; }
        td details > summary{cursor:pointer; color:#1e5ba8}
        ...
        a.link { color:#1e5ba8; text-decoration:none; }
...
<div class="container">
    ...
    <div class="card">
        <div class="card-header">
            <strong>Daftar Pengumpulan</strong>
        </div>
        <div class="card-body">
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>Nilai</th>
                        <th>Aksi</th>
                        <th>Komentar & Resubmit</th>
                    </tr>
                </thead>
                <tbody>
                <?php if (empty($submissions)): ?>
                    <tr><td colspan="12" style="text-align:center; color:#64748b; padding:16px;">Belum ada pengumpulan tugas.</td></tr>
                <?php else: $no=1; foreach($submissions as $s): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars(($s['nama_mapel']) ?? ''); ?></td>
                        <td><?php echo htmlspecialchars(($s['judul_tugas']) ?? ''); ?></td>
                        <td>
                        ...
                        <td><?php echo $s['file_path'] ? '<a class="link" href="../../assets/uploads/' . htmlspecialchars(($s['file_path']) ?? '') . '" target="_blank">Lihat</a>' : '-'; ?></td>
                        ...
                        <td>
                            <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap; margin-bottom:6px;">
                                <form method="post" style="display:inline" onsubmit="return confirm('Minta revisi untuk pengumpulan ini?');">
  ...
  box.innerHTML = arr.map(c=> `
    <div class=\"comment-item\">
      <div style=\"display:flex;justify-content:space-between;align-items:center\">
        ...
        <div>
          <button class=\"btn btn-secondary\" onclick=\"editComment(${pengumpulanId}, ${c.id}, \`${(c.comment||'').replace(/`/g,'\\`')}\`)\">Edit</button>
          <button class=\"btn btn-danger\" onclick=\"deleteComment(${pengumpulanId}, ${c.id})\" title="Hapus"><i class="fas fa-trash" aria-hidden="true"></i></button>
        </div>
      </div>
...
window.addEventListener('DOMContentLoaded', ()=>{
  document.querySelectorAll('[id^="cbox-"]').forEach(el=>{
    const id = parseInt(el.id.replace('cbox-',''));
