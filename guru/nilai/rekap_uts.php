...
function get_assignments_guru_rekap($guru_id){
    $sql = "SELECT ag.id, ag.kkm, ag.kelas_id, ag.mapel_id, ag.tahun_ajaran_id, 
                   k.nama_kelas, mp.nama_mapel, ta.nama_tahun_ajaran
...
function fetch_kontext($assignment_id){
    $res = query("SELECT ag.*, k.nama_kelas, mp.nama_mapel, ta.nama_tahun_ajaran 
                  FROM assignment_guru ag
                  JOIN kelas k ON k.id=ag.kelas_id
                  JOIN mata_pelajaran mp ON mp.id=ag.mapel_id
                  JOIN tahun_ajaran ta ON ta.id=ag.tahun_ajaran_id
...
$assignments = get_assignments_guru_rekap($guru_id);
$assignment_id = isset($_GET['assignment_id'])? (int)$_GET['assignment_id'] : 0;
$semester = isset($_GET['semester']) && in_array($_GET['semester'], ['Ganjil','Genap'], true) ? $_GET['semester'] : 'Ganjil';

// Handler POST harus berada sebelum output HTML
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['save_kkm'])) {
        $aid = (int)($_POST['assignment_id'] ?? 0);
        $kkm = (int)$_POST['kkm'];
        if ($aid>0 && $kkm>0) {
            $cek = query("SELECT id FROM assignment_guru WHERE id={$aid} AND guru_id={$guru_id} LIMIT 1");
            ...
            } else set_flash('error','Assignment tidak valid.');
        } else set_flash('error','KKM tidak valid.');
        $qs = http_build_query(['assignment_id'=>$aid,'semester'=>$semester]); redirect('rekap_uts.php?'.$qs);
    }
    // Simpan nilai manual (bulk)
    if (isset($_POST['save_bulk'])) {
        ...
        $updated = 0;
        foreach ($fmtArr as $sid => $v) {
            $sid = (int)$sid; $val = trim((string)$v); if ($val==='') continue;
            ...
            if ($rowC) query("UPDATE nilai SET nilai={$num}, updated_at='".date('Y-m-d H:i:s')."' WHERE id=".(int)$rowC['id']);
        ...
        $qs = http_build_query(['assignment_id'=>$assignment_id,'semester'=>$semester]); redirect('rekap_uts.php?'.$qs);
    }
    if (isset($_POST['import']) && isset($_FILES['import_file']) && $_FILES['import_file']['error']===UPLOAD_ERR_OK) {
        ...
        if ($h) {
            $delimiter = ',';
            $first = fgets($h);
            if ($first !== false && preg_match('/^sep=(.*)$/i', trim($first), $m)) {
                $sep = $m[1];
                if ($sep === '\\t' || $sep === "\t") $delimiter = "\t";
                elseif ($sep === ';') $delimiter = ';';
                elseif ($sep === ',') $delimiter = ',';
                $header = fgetcsv($h, 0, $delimiter);
            } else {
                if ($first !== false) { rewind($h); }
                $header = fgetcsv($h);
                if ($header && count($header)==1) { rewind($h); $header = fgetcsv($h, 0, ';'); $delimiter=';'; }
            }
            // Build flexible header map: normalize to alnum underscores and also index by contains
            $normHdr=[]; $idx_nis=null; $idx_fmt=null; $idx_uts=null;
            foreach ($header as $i=>$hn){
              $clean=preg_replace('/^\xEF\xBB\xBF/','', (string)$hn);
              $key=strtolower(preg_replace('/[^a-z0-9]+/i','_', trim($clean)));
              $normHdr[$key]=$i;
              if ($idx_nis===null && (strpos($key,'nis')!==false)) $idx_nis=$i; // prefer any col contains 'nis'
              if ($idx_fmt===null && (strpos($key,'formatif')!==false || strpos($key,'harian')!==false)) $idx_fmt=$i;
              if ($idx_uts===null && (strpos($key,'uts')!==false)) $idx_uts=$i;
            }
            $updated=0; $line=1;
            // If header didn't include nis via sep detection, try alternate delimiters
            if ($idx_nis===null){
              foreach (["\t",";",","] as $tryDelim){
                rewind($h);
                // skip possible sep line
                $first = fgets($h); if ($first===false) break; if (!preg_match('/^sep=/i',trim($first))) { rewind($h); }
                $header = fgetcsv($h,0,$tryDelim); if(!$header) continue;
                $normHdr=[]; $idx_nis=$idx_fmt=$idx_uts=null;
                foreach ($header as $i=>$hn){
                  $clean=preg_replace('/^\xEF\xBB\xBF/','', (string)$hn);
                  $key=strtolower(preg_replace('/[^a-z0-9]+/i','_', trim($clean)));
                  if ($idx_nis===null && strpos($key,'nis')!==false) $idx_nis=$i;
                  if ($idx_fmt===null && (strpos($key,'formatif')!==false || strpos($key,'harian')!==false)) $idx_fmt=$i;
                  if ($idx_uts===null && strpos($key,'uts')!==false) $idx_uts=$i;
                }
                if ($idx_nis!==null){ $delimiter=$tryDelim; break; }
              }
            }
            // read rows with chosen delimiter
            rewind($h);
            $first = fgets($h); if ($first===false) { set_flash('error','CSV kosong.'); $qs=http_build_query(['assignment_id'=>$assignment_id,'semester'=>$semester]); redirect('rekap_uts.php?'.$qs); }
            if (!preg_match('/^sep=/i',trim($first))) { rewind($h); }
            $header = fgetcsv($h,0,$delimiter);
            while(($row=fgetcsv($h,0,$delimiter))!==false){ $line++;
                $nis = '';
                if ($idx_nis!==null && isset($row[$idx_nis])) $nis = trim((string)$row[$idx_nis]);
                if($nis==='') continue;
                $fmt_in = ($idx_fmt!==null && isset($row[$idx_fmt]))? trim((string)$row[$idx_fmt]) : '';
                $uts_in = ($idx_uts!==null && isset($row[$idx_uts]))? trim((string)$row[$idx_uts]) : '';
                // exact match by users.nis first
                $qs=query("SELECT id, nis FROM users WHERE role='siswa' AND nis='".escape_string($nis)."' LIMIT 1");
                $rs=$qs?fetch_assoc($qs):null;
                if(!$rs){
                  // fallback remove dots both sides
                  $nis_flat=str_replace('.','',$nis);
                  $qr=query("SELECT id, nis FROM users WHERE role='siswa' AND REPLACE(nis,'.','')='".escape_string($nis_flat)."' LIMIT 1");
                  $rs=$qr?fetch_assoc($qr):null;
                }
                if(!$rs){ continue; }
                $siswa_id=(int)$rs['id'];
                if ($fmt_in!==''){
                    $val=(float)str_replace(',','.',$fmt_in);
                    $cek=query("SELECT id FROM nilai WHERE siswa_id={$siswa_id} AND mapel_id=".(int)($_POST['mapel_id']??0)." AND kelas_id=".(int)($_POST['kelas_id']??0)." AND tahun_ajaran_id=".(int)($_POST['tahun_ajaran_id']??0)." AND semester='".escape_string($semester)."' AND jenis_penilaian='Harian' LIMIT 1");
                    $rowC=$cek?fetch_assoc($cek):null;
                    if($rowC){ query("UPDATE nilai SET nilai=".$val.", updated_at='".date('Y-m-d H:i:s')."' WHERE id=".(int)$rowC['id']); }
                    else { query("INSERT INTO nilai (siswa_id,mapel_id,kelas_id,tahun_ajaran_id,semester,jenis_penilaian,nilai,created_at,updated_at) VALUES (".$siswa_id.",".(int)($_POST['mapel_id']??0).",".(int)($_POST['kelas_id']??0).",".(int)($_POST['tahun_ajaran_id']??0).",'".escape_string($semester)."','Harian',".$val.",'".date('Y-m-d H:i:s')."','".date('Y-m-d H:i:s')."')"); }
                    $updated++;
                }
                if ($uts_in!==''){
                    $val=(float)str_replace(',','.',$uts_in);
                    $cek=query("SELECT id FROM nilai WHERE siswa_id={$siswa_id} AND mapel_id=".(int)($_POST['mapel_id']??0)." AND kelas_id=".(int)($_POST['kelas_id']??0)." AND tahun_ajaran_id=".(int)($_POST['tahun_ajaran_id']??0)." AND semester='".escape_string($semester)."' AND jenis_penilaian='UTS' LIMIT 1");
                    $rowC=$cek?fetch_assoc($cek):null;
                    if($rowC){ query("UPDATE nilai SET nilai=".$val.", updated_at='".date('Y-m-d H:i:s')."' WHERE id=".(int)$rowC['id']); }
                    else { query("INSERT INTO nilai (siswa_id,mapel_id,kelas_id,tahun_ajaran_id,semester,jenis_penilaian,nilai,created_at,updated_at) VALUES (".$siswa_id.",".(int)($_POST['mapel_id']??0).",".(int)($_POST['kelas_id']??0).",".(int)($_POST['tahun_ajaran_id']??0).",'".escape_string($semester)."','UTS',".$val.",'".date('Y-m-d H:i:s')."','".date('Y-m-d H:i:s')."')"); }
                    $updated++;
                }
            }
            fclose($h);
            set_flash('success','Import selesai. Baris diproses: '.$updated);
            $qs=http_build_query(['assignment_id'=>$assignment_id,'semester'=>$semester]); redirect('rekap_uts.php?'.$qs);
        } else {
...
if ($ctx && isset($_GET['export_csv']) && (int)$_GET['export_csv']===1) {
    ...
    $no=1; foreach ($rows as $r){
        ...
        $line = [
            ...
            $r['nama_lengkap'],
            (string)$kkm_val,
            $fmt!==null? number_format($fmt,2) : '',
            to_predikat($fmt),
            $uts!==null? number_format($uts,2) : '',
            to_predikat($uts),
            $ket
        ];
...
if ($ctx && isset($_GET['export']) && (int)$_GET['export']===1) {
    // Export ke XLS (HTML table) agar per kolom sesuai template admin
    $rows = fetch_siswa_kelas((int)$ctx['kelas_id']);
    ...
    $no=1; foreach ($rows as $r){
        ...
        echo '<tr>'
            .'<td>'.($no++).'</td>'
            .'<td>'.htmlspecialchars($nis, ENT_QUOTES, 'UTF-8').'</td>'
...
<head>
  ...
  <title>Rekap Nilai UTS - Guru</title>
  ...
  <style>
    ...
    @media print {
      ...
      .btn, .alert, .muted { display: none !important; }
...
<body>
  <div class="container">
    <div class="card">
      <div class="card-header">Rekap Nilai UTS</div>
      <div class="card-body">
        ...
        <?php if (!$assignments): ?>
            <div class="muted">Anda belum memiliki assignment. Minta admin untuk menambahkan assignment_guru.</div>
        <?php else: ?>
        <form method="get" action="rekap_uts.php" class="row" style="align-items:flex-end;margin-bottom:12px">
          <div class="col">
            <label>Pilih Assignment</label>
            <select name="assignment_id" required>
              <option value="">- pilih -</option>
              <?php foreach ($assignments as $a): $sel = ($assignment_id==(int)$a['id'])? 'selected':''; ?>
                <option value="<?php echo (int)$a['id']; ?>" <?php echo $sel; ?>><?php echo htmlspecialchars(($a['nama_mapel'].' - '.$a['nama_kelas'].' ('.$a['nama_tahun_ajaran'].')') ?? ''); ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="col">
        ...
        <?php if ($ctx): ?>
          <form method="post" enctype="multipart/form-data" class="row" style="align-items:flex-end;margin-bottom:12px; gap:12px">
            ...
            <?php if ($ctx): ?>
            <input type="hidden" name="mapel_id" value="<?php echo (int)$ctx['mapel_id']; ?>" />
            <input type="hidden" name="kelas_id" value="<?php echo (int)$ctx['kelas_id']; ?>" />
          ...
          <?php
            // Handle import
            if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['import']) && isset($_FILES['import_file']) && $_FILES['import_file']['error']===UPLOAD_ERR_OK) {
              $tmp = $_FILES['import_file']['tmp_name'];
              $h = fopen($tmp,'r');
              if ($h) {
                $header = fgetcsv($h);
                if ($header && count($header)==1) { rewind($h); $header = fgetcsv($h, 0, ';'); $delimiter=';'; } else { $delimiter=','; }
                $map=[]; foreach ($header as $i=>$hn){ $key=strtolower(trim(preg_replace('/\s+/', '_', $hn))); $map[$key]=$i; }
                $get=function($row,$name)use($map){ $idx=isset($map[$name])?$map[$name]:null; return ($idx!==null && isset($row[$idx]))? $row[$idx]:''; };
                $updated=0; $line=1;
                while(($row=fgetcsv($h,0,$delimiter))!==false){ $line++;
                  $nis=trim((string)$getV($row,['nis','nisn'])); if($nis==='') continue;
                  $fmt_in=trim((string)$getV($row,['formatif','nilai_formatif','harian'])); $uts_in=trim((string)$getV($row,['uts','nilai_uts']));
                  $qs=query("SELECT id FROM users WHERE role='siswa' AND (nisn='".escape_string($nis)."' OR nis='".escape_string($nis)."') LIMIT 1");
                  $rs=$qs?fetch_assoc($qs):null; if(!$rs) continue; $siswa_id=(int)$rs['id'];
                  if ($fmt_in!==''){
                    $val=(float)$fmt_in;
                    $cek=query("SELECT id FROM nilai WHERE siswa_id={$siswa_id} AND mapel_id=".(int)$ctx['mapel_id']." AND kelas_id=".(int)$ctx['kelas_id']." AND tahun_ajaran_id=".(int)$ctx['tahun_ajaran_id']." AND semester='".escape_string($semester)."' AND jenis_penilaian='Harian' LIMIT 1");
                    $rowC=$cek?fetch_assoc($cek):null;
                    if($rowC){ query("UPDATE nilai SET nilai=".$val.", updated_at='".date('Y-m-d H:i:s')."' WHERE id=".(int)$rowC['id']); }
                    else { query("INSERT INTO nilai (siswa_id,mapel_id,kelas_id,tahun_ajaran_id,semester,jenis_penilaian,nilai,created_at,updated_at) VALUES (".$siswa_id.",".(int)$ctx['mapel_id'].",".(int)$ctx['kelas_id'].",".(int)$ctx['tahun_ajaran_id'].",'".escape_string($semester)."','Harian',".$val.",'".date('Y-m-d H:i:s')."','".date('Y-m-d H:i:s')."')"); }
                    $updated++;
                  }
                  if ($uts_in!==''){
                    $val=(float)$uts_in;
                    $cek=query("SELECT id FROM nilai WHERE siswa_id={$siswa_id} AND mapel_id=".(int)$ctx['mapel_id']." AND kelas_id=".(int)$ctx['kelas_id']." AND tahun_ajaran_id=".(int)$ctx['tahun_ajaran_id']." AND semester='".escape_string($semester)."' AND jenis_penilaian='UTS' LIMIT 1");
                    $rowC=$cek?fetch_assoc($cek):null;
                    if($rowC){ query("UPDATE nilai SET nilai=".$val.", updated_at='".date('Y-m-d H:i:s')."' WHERE id=".(int)$rowC['id']); }
                    else { query("INSERT INTO nilai (siswa_id,mapel_id,kelas_id,tahun_ajaran_id,semester,jenis_penilaian,nilai,created_at,updated_at) VALUES (".$siswa_id.",".(int)$ctx['mapel_id'].",".(int)$ctx['kelas_id'].",".(int)$ctx['tahun_ajaran_id'].",'".escape_string($semester)."','UTS',".$val.",'".date('Y-m-d H:i:s')."','".date('Y-m-d H:i:s')."')"); }
                    $updated++;
                  }
                }
                fclose($h);
                if($updated>0) set_flash('success','Import selesai. Baris diproses: '.$updated); else set_flash('error','Import selesai. Tidak ada baris yang diproses.');
                $qs=http_build_query(['assignment_id'=>$assignment_id,'semester'=>$semester]); redirect('rekap_uts.php?'.$qs);
              } else { set_flash('error','Gagal membaca file.'); }
            }
          ?>
